...
# Show the graph given adacency matrix (obviously only usefull for small graphs)
...
(This is so sklearn clustering metrics can be used)
"""
...
# Perform Symmetric NMF clustering on the graph (deprecated)
...
# A type of "force" clustering (think there is actually a name for this). 
# Essentially place nodes under a force field which pulls together connected nodes
# and moves unconnected ones appart. Then use mean shift clustering. Works well, but way
# slower then label propagation, and the results are comparable.
...
def run_cluster_test(graph: np.ndarray, idx_to_instance: Dict, ground_truth, task : Literal['force_opt', 'sym_nmf', 'vis', 'lovain', 'lpa', 'lpatorch'] = 'force_opt', verbose=True):
    if task == 'vis':
        print("Visualization")
    ...
    if task == 'force_opt' or task == 'lpatorch':
        if task == 'force_opt':
            ...
            r = lp_cluster_torch(graph).cpu().numpy()
        u = {}
