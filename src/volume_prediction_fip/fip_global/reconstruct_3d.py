"""
Generate a 3d reconstruction of a fip scan using openMVS. 
Requires a configuration file of generate_scaled_calibration. Note that the 
calibration can be reused for different scans as long as nothing changes significantly
on the FIP. (I.e. a calibration can be done on one scan and reused for weeks or months in theory)
openMVG is required for the conversion of the openMVG Sfm format to openMVS. Saves depthmaps as numpy 
arrays and the final pointcloud in the output directory. 

This is SLOW! -> Around 3-4 minutes for a scan even with GPU.
"""
