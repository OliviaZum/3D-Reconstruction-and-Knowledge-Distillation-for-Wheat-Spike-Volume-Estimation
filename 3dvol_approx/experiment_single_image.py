...
def evaluate(dataloader: DataLoader, model: nn.Module, show_plot = False, should_print=True):
    ...
    with torch.no_grad():
        ...
        for images, plant, imagemask, label, weight in dataloader:
            ...
            #conf = ((conf - conf[~imagemask].min()) / (conf[~imagemask].max() - conf[~imagemask].min())) * 4.3 - 1.8
            ...
            input[imagemask] = 0
            input = torch.sum(input, dim=1, keepdim=True) / msum
            """
            input, vol_prob = max_prob_estimate(input, conf, (torch.ones(input.shape[1], device=input.device) / input.shape[1]).unsqueeze(0), imagemask)

            combined_probs.append(vol_prob.cpu())
            vol_pred.append(input.cpu().squeeze())
        ...
        if show_plot:
            ...
            plt.scatter(torch.abs(datasets_3d.vol_unorm(vol_real) - datasets_3d.vol_unorm(vol_pred)), combined_probs)
            plt.show()
...
if __name__ == "__main__":
    ...
    for epoch in range(150):
        ...
        if lossfn.is_var_penalty and (best_val is None or err_val < best_val):
            best_val = err_val
