...
def main(openmvg_path, fip_extrinsics_folder):
    ...
    subprocess.run([
        os.path.join(openmvg_path, 'openMVG_main_SfMInit_ImageListing.exe'),
        '-i', './build/images', '-o', 'build', '-f', '10200', '-g', '0'
    ], check=True)

    subprocess.run([
        os.path.join(openmvg_path, 'openMVG_main_ComputeFeatures.exe'),
        '-i', 'build/sfm_data.json', '-o', 'build'
    ], check=True)

    subprocess.run([
        os.path.join(openmvg_path, 'openMVG_main_PairGenerator.exe'),
        '-i', 'build/sfm_data.json', '-o', 'build/pairs.txt', '-m', 'EXHAUSTIVE'
    ], check=True)

    subprocess.run([
        os.path.join(openmvg_path, 'openMVG_main_ComputeMatches.exe'),
        '-i', 'build/sfm_data.json', '-o', 'build/matches.putative.bin', '-p', 'build/pairs.txt'
    ], check=True)

    subprocess.run([
        os.path.join(openmvg_path, 'openMVG_main_GeometricFilter.exe'),
        '-i', 'build/sfm_data.json', '-m', 'build/matches.putative.bin', '-o', 'build/matches.geom.bin'
    ], check=True)

    subprocess.run([
        os.path.join(openmvg_path, 'openMVG_main_SfM.exe'),
        '-i', 'build/sfm_data.json', '-o', 'build', '-M', 'build/matches.geom.bin',
        '--sfm_engine', 'INCREMENTAL'
    ], check=True)

    subprocess.run([
        os.path.join(openmvg_path, 'openMVG_main_ConvertSfM_DataFormat.exe'),
